/**
 * Anilibria Auto-Skip Plugin v2.1.3
 *
 * Универсальный плагин для автоматического пропуска заставок и титров в аниме от всех источников.
 *
 * ОСНОВНЫЕ ИСПРАВЛЕНИЯ v2.1.3:
 * - Значительно улучшена точность определения номера эпизода за счет:
 * - Усиленного приоритета данных из Lampa.Activity и Lampa.Player.
 * - Расширенного "черного списка" для игнорирования не-эпизодных чисел (версии, даты, таймкоды, время).
 * - Контекстной проверки DOM-элементов для подтверждения, что число относится к эпизоду.
 * - Более надежной логики "стабильного" номера эпизода.
 * - Удалены неработающие API-запросы из-за CORS ограничений (подтверждено и объяснено).
...
(function() {
    ...
    const CONFIG = {
        ...
        version: '2.1.3',
        api: {
            // API запросы отключены из-за CORS ограничений браузера.
            // Прямые запросы к внешним API из Lampa (работающей в iframe)
            // часто блокируются из-за политики безопасности Cross-Origin Resource Sharing (CORS).
    ...
    class AnilibriaAutoSkipPlugin {
        ...
        init() {
            try {
                this.log('🚀 Инициализация универсального плагина v2.1.3...', 'info');
                this.loadSettings();
                ...
                this.showSkipNotification('success', '🎯 Anilibria Auto-Skip v2.1.3 готов для всех источников!');
        ...
        findUniversalEpisodeNumber() {
            ...
            if (lampaEpisode !== null) {
                ...
                // Если найден надежный эпизод, сразу обновляем стабильный и возвращаем
                if (foundEpisode !== this.stableEpisodeNumber) {
                    this.stableEpisodeNumber = foundEpisode;
                    this.lastEpisodeChangeTime = now;
                    this.log(`🎯 Стабильный результат обновлен (Lampa Activity): сезон ${foundSeason}, эпизод ${foundEpisode}`, 'info');
                }
        ...
        extractEpisodeFromElement(element) {
            ...
            // Черный список паттернов для игнорирования
            const junkTextPatterns = [
                /v\d+\.\d+\.\d+/, // Версии типа "v1.4.9"
                /\d{2}\.\d{2}\.\d{4}/, // Даты типа "24.07.2025"
                /\d{2}:\d{2}:\d{2}/, // Таймкоды типа "00:01:54"
                /online_mod/i, // Название плагина
                /bwa/i, // Название плеера
                /фильм/i, // Слово "фильм"
                /сезон/i, // Слово "сезон"
                /эпизодов/i, // Слово "эпизодов"
                /просмотрено/i, // Слово "просмотрено"
                /720p|1080p|4k/i, // Качество видео
                /м\.\s*из\s*м\./i, // "м. из м."
                /с\.\s*из\s*с\./i, // "с. из с."
                /\d+%/ // Проценты
            ];

            // Проверяем, не является ли текст "мусорным"
            if (junkTextPatterns.some(pattern => pattern.test(text))) {
                this.log(`DEBUG: Игнорируем мусорный текст: "${text}"`, 'debug');
                return null;
            }

            // Различные паттерны для извлечения номера эпизода (от наиболее специфичных к менее)
            const patterns = [
                /серия\s*(\d+)/i, // "серия N"
            ...
            // Проверяем, что число не слишком большое (не год) и элемент имеет класс, указывающий на эпизод
            const isEpisodeElement = (element.className.includes('episode') || element.className.includes('series') ||
                                      element.className.includes('item') || element.className.includes('selector') ||
                                      element.id.includes('episode') || element.id.includes('series'));

            if (!isNaN(pureNum) && pureNum > 0 && pureNum < 1000 && text.length <= 4 && isEpisodeElement) {
                this.log(`DEBUG: Эпизод из чистого числового текста с проверкой класса/ID: "${text}" -> ${pureNum}`, 'debug');
                return pureNum;
            }
                ...
                if (Lampa.Activity && Lampa.Activity.active()) {
                    ...
                    if (activity && activity.component) {
                        ...
                        const episodeSources = [
                            ...
                            () => Lampa.Player?.get()?.playlist?.current?.episode, // Из плейлиста плеера
                            () => Lampa.Player?.get()?.playlist?.current?.number, // Из плейлиста плеера
                            () => component.episode,
        ...
        performExtendedDiagnostics() {
            this.log('🔍 === РАСШИРЕННАЯ ДИАГНОСТИКА v2.1.3 ===', 'info');
        ...
        log(message, level = 'info') {
            ...
            const isEpisodeDebug = message.includes('Эпизод') || message.includes('Результат поиска') || message.includes('DEBUG');
