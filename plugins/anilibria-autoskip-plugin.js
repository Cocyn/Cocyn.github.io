/**
 * Anilibria Auto-Skip Plugin v2.1.2
 *
 * Универсальный плагин для автоматического пропуска заставок и титров в аниме от всех источников.
 *
 * ОСНОВНЫЕ ИСПРАВЛЕНИЯ v2.1.2:
 * - Значительно улучшена точность определения номера эпизода за счет более строгих правил парсинга DOM.
 * - Добавлен "черный список" для игнорирования не-эпизодных чисел (версии, даты, таймкоды).
 * - Усилен приоритет данных из Lampa.Activity и Lampa.Player для определения эпизода.
 * - Дополнительные проверки и логирование для отладки определения эпизодов.
...
(function() {
    ...
    class AnilibriaAutoSkipPlugin {
        ...
        init() {
            try {
                this.log('🚀 Инициализация универсального плагина v2.1.2...', 'info');
                this.loadSettings();
                ...
                this.showSkipNotification('success', '🎯 Anilibria Auto-Skip v2.1.2 готов для всех источников!');
        ...
        extractEpisodeFromElement(element) {
            ...
            if (!isNaN(pureNum) && pureNum > 0 && pureNum < 1000 && text.length <= 4 &&
                (element.className.includes('episode') || element.className.includes('series') ||
        ...
        performExtendedDiagnostics() {
            this.log('🔍 === РАСШИРЕННАЯ ДИАГНОСТИКА v2.1.2 ===', 'info');
